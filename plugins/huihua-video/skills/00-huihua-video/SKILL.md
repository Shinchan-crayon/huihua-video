...
**默认模式：成品执行模式。** 用户调用 `$huihua-video` 后，只要当前请求是在制作、继续、重做或交付视频，就立即按唯一默认流程执行到 MP4。用户只说“自由发挥”“继续”或提供一篇文章也属于完整生产指令；自动补齐选题、口播、风格、比例、时长和输出目录，不要求用户重复写长指令。

只有当前请求明确要求修改、开发、测试或发布 `huihua-video` 插件源码，才进入开发模式。此前聊过插件开发、当前工作目录位于插件仓库、用户要求分析报错，均不构成开发授权。制作与开发同时出现时严格按用户点名的范围处理，不把生产任务扩展为插件修改。

运行失败不得切换为开发模式，不得因此读取、修改或测试插件源码。只按生产规则自动重试；达到既定重试上限后报告最终错误并停止。不得自行创建开发目录、测试项目、开发日志、提交或版本变更。

用户未指定输出目录时，在 `~/Documents/huihua-video-runs/<项目名>-<时间戳>/` 创建独立生产项目。不得把生产项目建在插件源码仓库或 Shin-video 目录中。
...
1. 运行 `python3 ../../scripts/initialize_huihua_project.py --project-dir <project-dir>`，只建立独立项目目录和 `.huihua-video-runtime/`；未指定目录时使用成品执行模式的默认输出目录。
