...
首次缺少生图配置时，调用 `$image-prompt-generator` 的首次配置流程，设置默认生图模型。图片渠道使用自己的 API Key，不得读取、复用或写入音频模型 API Key。
...
6. 调用 `$huihua-image-director` 和 `$image-prompt-generator` 直接并发生成图片；不展示图片规划或 Prompt。
...
- 场景切换和字幕切换彼此独立。
- 生图必须调用 `$image-prompt-generator`；每条图片 Prompt 必须显式写入 `style-profile.json.aspect_ratio`。
- 生图并发上限为 3 张。失败图片自动重试，单张连续 3 次仍未返回图片时才向用户报告最终错误。
- 所有插画必须完整显示，禁止裁切主体。
