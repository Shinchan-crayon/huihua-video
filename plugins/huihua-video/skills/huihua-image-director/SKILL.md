...
仅处理 `workflow-state.json.product_id == "huihua-video"` 的当前项目。图片和清单必须留在当前项目内，不得读取或复用 Shin-video 仓库、`.shin-video-runtime`、旧图片和缓存。
...
- 每张图片生成后必须运行 `python3 ../../scripts/probe_image.py <project-dir> <image>` 记录实际宽高。
